...
struct FstrlenSSE42 {
#if 1
	static const char *(*find)(const char *, size_t);
#else
	static inline const char *find(const char *p, size_t)
	{
		return p_strlenSSE42(p) + p;
	}
#endif
};
const char * (*FstrlenSSE42::find)(const char*, size_t) = (const char *(*)(const char*,size_t))strlenSSE42_code.getCode();
...
//	const int aveTbl[] = { 64, 128, 256, 512, 1024 };
	...
	for (size_t i = 0; i < NUM_OF_ARRAY(aveTbl); i++) {
		...
		puts("strlenSSE42");
		ret = test<FstrlenSSE42>(begin, N, count);
		if (ret.hit != hit) { printf("ERROR!!! ok=%d, ng=%d\n", hit, ret.hit); }
		ret.put();
		rv[2].push_back(ret);
	...
	printf("\n");
	static const char nameTbl[funcNum][16] = { "strlenANSI ", "strlenSSE2 ", "strlenSSE42", "memchrANSI ", "memchrSSE2 " };
